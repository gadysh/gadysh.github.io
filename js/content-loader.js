...
// מיפוי אייקונים - מתרגם emoji ושמות לנתיבי SVG
function getIconPath(iconHint) {
    // מיפוי בסיסי מemoji לSVG
    const iconMap = {
        '💬': 'chat.svg',
        '⚖️': 'document.svg',
        '👥': 'users.svg',
        ...
        '✅': 'check.svg',
        '🤖': 'grid.svg', // default
        // תמיכה גם בשמות מפורשים
        'customer-service': 'chat.svg',
        'legal': 'document.svg',
        'hr': 'users.svg',
        'voice': 'mic.svg',
        'analytics': 'chart.svg',
        'knowledge': 'book.svg',
        'procurement': 'cart.svg',
        'quality': 'tool.svg'
    };

    // אם זה emoji ישיר
    if (iconMap[iconHint]) {
        return `/assets/icons/${iconMap[iconHint]}`;
    }

    // נסה למצוא לפי substring
    const key = Object.keys(iconMap).find(k => iconHint.toLowerCase().includes(k));
    if (key) {
...
export async function loadUseCases() {
    ...
    for (const file of files) {
        try {
            ...
            let title = '';
            let oneLiner = '';
            let iconHint = file.split('-')[1]; // fallback לשם קובץ
            let tags = [];
            ...
            if (iconMatch) iconHint = iconMatch[1];

            const tagsMatch = markdown.match(/<!--\s*tags:\s*(.+?)\s*-->/);
            if (tagsMatch) tags = tagsMatch[1].split(',').map(t => t.trim());

            // קבל נתיב אייקון SVG
            const iconPath = getIconPath(iconHint);

            useCases.push({
                id: file.replace('.md', ''),
                title,
                oneLiner,
                iconPath, // SVG path במקום emoji
                tags,
                content,
                file
            });
